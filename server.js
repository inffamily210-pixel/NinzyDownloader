...
// PENTING: tanpa ini, error yang lolos dari try-catch (misal Promise reject
// tanpa .catch(), atau exception di luar blok try) bikin Node.js CRASH
// TOTAL secara diam-diam -- prosesnya mati, Railway auto-restart container,
// tapi crash-nya sendiri gak sempat ke-log dengan jelas (kadang cuma
// keliatan sebagai gap kosong di log, bukan pesan error). Ini persis pola
// yang bikin error 500 muncul di browser (Railway proxy generate 500 pas
// container lagi restart) TAPI gak ada log spesifik yang nunjukin
// penyebabnya. Dua handler ini nangkep exception itu SEBELUM Node.js
// crash, nge-log detailnya, dan (untuk unhandledRejection) bikin proses
// TETAP HIDUP -- jadi 1 request yang error gak matiin seluruh server buat
// semua platform lain yang lagi jalan bersamaan.
process.on('uncaughtException', (err) => {
  console.error('[FATAL uncaughtException]', err && err.stack || err);
  // uncaughtException tetap fatal secara desain Node.js (state proses udah
  // gak bisa dipercaya lagi) -- biarkan proses exit, tapi Railway bakal
  // auto-restart, dan sekarang minimal ada log yang jelas sebelum itu.
  process.exit(1);
});
process.on('unhandledRejection', (reason) => {
  console.error('[unhandledRejection]', reason && reason.stack || reason);
  // SENGAJA gak process.exit() di sini -- promise reject yang gak
  // ke-catch itu jauh lebih umum terjadi (misal 1 platform gagal fetch)
  // dan gak selalu berarti state proses rusak. Biarkan proses tetap hidup
